...
class AcrobotEmaEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # Create spaces
        high = np.array([1.0, 1.0, self.MAX_ANG_2])
    ...
    def _get_obs(self):
        ...
        if self.observe is None:
            self.observe = np.array([np.cos(s[0]), np.sin(s[0]), s[1]])
        else:
            self.observe = self.POMDP_PARAM*self.observe + (1.0-self.POMDP_PARAM)* np.array([np.cos(s[0]), np.sin(s[0]), s[1]])
        return self.observe
