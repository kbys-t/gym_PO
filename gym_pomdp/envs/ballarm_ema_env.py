...
Observation is limited to Positions.
Observation is updated through EMA filter with 1-self.POMDP_PARAM.
"""
...
class BallArmEmaEnv(gym.Env):
    ...
    def __init__(self):
        ...
        # Create spaces
        high_obs = np.array([self.MAX_ANG, self.MAX_ANG, self.MAX_OBJ_POS, self.MAX_OBJ_POS])
        ...
        # Prepare parameter to decide the degree of POMDP
        self.POMDP_PARAM = 0.0
    ...
    def _step(self, action):
        ...
        if any(collision):
            ...
            reward += ( np.exp( - 2.0 * np.linalg.norm(pt - ns[4:6]) ) - 0.5 ) * 2.0
    ...
    def _get_obs(self):
        ...
        if self.observe is None:
            self.observe = np.array([s[0], s[1], s[4], s[5]])
        else:
            self.observe = self.POMDP_PARAM*self.observe + (1.0-self.POMDP_PARAM)* np.array([s[0], s[1], s[4], s[5]])
        return self.observe
    ...
    def _dynamics(self, s, a, dt):
        ...
        p2 = p1 + dp
        d1 = np.absolute( p1[1]*pc[0] - p1[0]*pc[1] ) / np.sqrt( p1[1]**2 + p1[0]**2 )
        d2 = np.absolute( dp[1]*pc[0] - dp[0]*pc[1] + p2[0]*p1[1]-p2[1]*p1[0] ) \
            / np.sqrt( dp[1]**2 + dp[0]**2 )
        i1 = ( p1[0]**2*pc[0] + p1[0]*p1[1]*pc[1] ) / ( p1[0]**2 + p1[1]**2 )
        i2 = ( dp[0]**2*pc[0] + dp[1]**2*p1[0] + dp[0]*dp[1]*(pc[1] - p1[1]) ) / ( dp[0]**2 + dp[1]**2 )
        # judge
        if d2 <= 2.0 * r and i2 >= np.minimum(p1[0], p2[0]) and i2 <= np.maximum(p1[0], p2[0]):
            mg = m1 + m2
            pgn = np.array([\
            ...
            ns[4] += ns[6]*dt
            ns[5] += ns[7]*dt
        elif d1 <= 2.0 * r and i1 >= np.minimum(0.0, p1[0]) and i1 <= np.maximum(0.0, p1[0]):
            mg = m1
